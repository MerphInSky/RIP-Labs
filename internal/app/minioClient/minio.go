...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, componentID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("component_%d%s", componentID, ext)
...
func UploadVideo(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, componentID uint) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "", err
	}
	...
	objectName := fmt.Sprintf("component_%d_video%s", componentID, ext)
